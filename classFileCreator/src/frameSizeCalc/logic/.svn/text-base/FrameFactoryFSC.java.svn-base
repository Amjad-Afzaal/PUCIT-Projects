 ...
 * @author Faisal Aslam
 */
public class FrameFactoryFSC extends VerificationFrameFactory {

    private static final VerificationFrameFactory frameFactory = new FrameFactoryFSC();
    ...
    public OperandStack createOperandStack(int maxStack) {
        return new OperandStackFSC(maxStack);
    }

    @Override
    public LocalVariables createLocalVariables(int maxStack) {
        return new LocalVariablesFSC(maxStack);
    }
    
   
    @Override
    public DataFlowAnalyzer createDataFlowAnalyzer() {
        return DataFlowAnalyzerFSC.getInstanceOf();
    }
}
