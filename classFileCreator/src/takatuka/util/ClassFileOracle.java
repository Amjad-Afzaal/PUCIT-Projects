...
class ClassFileOracle {
    ...
    final String getUTF8(int index, MultiplePoolsFacade cp) {
        UTF8Info utf8Info = (UTF8Info) cp.get(index, CPTagValues.CONSTANT_Utf8);
    ...
    final Vector<ClassFile> getAllDirectImplementations(String fullyQualifiedInterfaceName) {
        ...
        for (int loop = 0; loop < size; loop++) {
            ...
            for (int interLoop = 0; interLoop < interfaceCont.getCurrentSize(); interLoop++) {
                ...
                ClassInfo cInfo = (ClassInfo) cp.get(cpIndex, CPTagValues.CONSTANT_Class);
    ...
    final ClassFile getSuperClass(ClassFile cFile) {
        ...
        ClassInfo superCI = (ClassInfo) cp.get(superThis, CPTagValues.CONSTANT_Class);
    ...
    private void getAllInterfacesImpledHelper(ClassFile cFile, Vector<ClassFile> interfaces) throws Exception {
        MultiplePoolsFacade pOne = cFile.getConstantPool();
    ...
    final ClassFile getClass(int thisClass, MultiplePoolsFacade cp) {
        ...
        Object cpObj = cp.get(thisClass, CPTagValues.CONSTANT_Class);
    ...
    final int getNearestCommonSuperClass(int classA, int classB) throws
        ...
        MultiplePoolsFacade pOne = null; //todo
        if (classA == classB) {
            return classA;
        }
...
//                Miscellaneous.println("return here " + bsSuper.getActualClassName());
     ...
     */
    final int getClassInfoByName(String name, MultiplePoolsFacade cp) {
        try {
            for (int loop = 1; loop < cp.getCurrentSize(); loop++) {
                InfoBase base = (InfoBase) cp.get(loop);
                if (base instanceof ClassInfo) {
                    ClassInfo cInfo = (ClassInfo) base;
                    String className = null;//cInfo.getClassName();
                    if (className == null) {
                        Oracle oracle = Oracle.getInstanceOf();
                        int cpIndex = cInfo.getIndex().intValueUnsigned();
                        className = oracle.getUTF8(cpIndex, cp);
                    }
    ...
    private String getClassName(int thisPointer, MultiplePoolsFacade cp) {
        ClassInfo cInfo = (ClassInfo) cp.get(thisPointer, CPTagValues.CONSTANT_Class);
    ...
    final int getMethodInfoIndexFromContr(ClassFile cFile, String methodName,
        ...
        for (int loop = 0; loop < mCont.getCurrentSize(); loop++) {
            ...
            UTF8Info utf8Name = (UTF8Info) localCP.get(mInfo.getNameIndex().
                    intValueUnsigned(), CPTagValues.CONSTANT_Utf8);
    ...
    final int getReferenceFieldFromClassFile(MultiplePoolsFacade cp,
        ...
        NameAndTypeInfo nAt = (NameAndTypeInfo) cp.get(nATIndex, CPTagValues.CONSTANT_NameAndType);
