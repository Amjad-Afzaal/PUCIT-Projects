...
public class BHInstruction extends Instruction {
    ...
    protected void init() {
        ...
        if (prevInst == null && getParentCodeAtt() != null && !getParentCodeAtt().getInstructions().isEmpty()) {
            CodeAtt parentCAtt = getParentCodeAtt();
            prevInst= (BHInstruction) parentCAtt.getInstructions().get(parentCAtt.getInstructions().size() -1);
        }
        if (getParentCodeAtt() != null) {
            BHCodeAtt.currentCodeAtt = (BHCodeAtt) getParentCodeAtt();
        }
