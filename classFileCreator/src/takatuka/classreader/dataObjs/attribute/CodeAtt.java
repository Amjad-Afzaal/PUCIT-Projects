...
public class CodeAtt extends AttributeInfo {
    ...
    private static FactoryFacade factory = FactoryPlaceholder.getInstanceOf().
            getFactory();
    private Un max_stack = factory.createUn(0).trim(2); //(2);
    private Un max_locals = factory.createUn(0).trim(2); //(2);
    private Un code_length; //(4);
    private Un code; //u1 arrary of size code_length
    private Vector instructions = new Vector();
    private Un exception_table_length = factory.createUn(0).trim(2); //(2);
    ...
    private AttributeInfoController attributes = new AttributeInfoController(0);
    ...
    public CodeAtt() throws Exception {
    }
    ...
    public CodeAtt(Un u2_attrNameIndex, Un u4_attributeLength, Un u2_maxStack,
        ...
        if (u2_maxLocals != null) {
            setMaxLocals(u2_maxLocals);
        }
        if (u4_codeLength != null) {
    ...
    public void setInstructions(Vector instructions) {
        try {
            ...
            addNewInstrIdRecord();
            removeNumberAtt();
            int attLength = 12 + getCodeLength().intValueSigned();
            AttributeInfoController attContrl = getAttributes();
            for (int loop = 0; loop < attContrl.getCurrentSize(); loop++) {
                AttributeInfo attInfo = (AttributeInfo) attContrl.get(loop);
                attLength += 6; //for name and lenght itself
                attLength += attInfo.getAttributeLength().intValueSigned();
            }
            FactoryFacade factoryFacade = FactoryPlaceholder.getInstanceOf().getFactory();
            Un attLengthUn = factoryFacade.createUn(attLength).trim(4);
            setAttributeLength(attLengthUn);
        } catch (Exception d) {
            d.printStackTrace();
            System.exit(1);
        }
    ...
    private void removeNumberAtt() {
        try {
            //if (true) return;
            AttributeInfoController contr = getAttributes();
            contr.clear();
            for (int index = 0; index < contr.getCurrentSize(); index++) {
                AttributeInfo att = (AttributeInfo) contr.get(index);
                if (att instanceof LineNumberTableAtt) {
                    contr.remove(att);
                    break;
                }
            }
    ...
    public int updateCodeLength() {
        ...
        try {
            setCodeLength(factory.createUn(codeLength).trim(4));
        } catch (Exception d) {
            d.printStackTrace();
            Miscellaneous.exit();
        }
    ...
    public void setMaxStack(int maxStack) throws UnSizeException {
        try {
            FactoryFacade factoryFacade = FactoryPlaceholder.getInstanceOf().getFactory();
            
            this.max_stack = factoryFacade.createUn(maxStack).trim(2);
        } catch (Exception d) {
            d.printStackTrace();
            System.exit(1);
        }
    ...
    public void setMaxLocals(int maxLocals) throws UnSizeException {
        try {
            FactoryFacade factoryFacade = FactoryPlaceholder.getInstanceOf().getFactory();
            
            this.max_locals = factoryFacade.createUn(maxLocals).trim(2);
        } catch (Exception d) {
            d.printStackTrace();
            System.exit(1);
        }
...
//        code ;//(this.code_length.intValueUnsigned());
    ...
    public void restorePCInfoInExceptionTables(Instruction inst) {
        try {
