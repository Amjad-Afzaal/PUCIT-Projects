...
public class InterfaceController extends ControllerBase {
     ...
     * @param size int
     */
    public InterfaceController(int size) {
        super(size);
        super.setAllowedClassName(Un.class);
    }
    ...
    public String writeSelected(BufferedByteCountingOutputStream buff) throws
        ...
        for (int loop = 0; loop < getCurrentSize(); loop++) {
            ret = ret + ((Un) get(loop)).writeSelected(buff);
        }
        return ret;
    }
