...
public class MethodInfo extends FieldInfo {
    ...
    public String writeSelected(BufferedByteCountingOutputStream buff) throws
        ...
        String name = oracle.methodOrFieldName(this, this.getClassFile().getConstantPool());
        String desc = oracle.methodOrFieldDescription(this, this.getClassFile().getConstantPool());
        return "\n\tMethodInfo=[name=" +name+desc+", "+super.writeSelected(buff);
    }
 ...
 */
    ...
    public static Instruction findPreviousInstr(long id, Vector<Instruction> instrs) {
        ...
        for (int loop = 0; loop < instrs.size(); loop++) {
            ret = (Instruction) instrs.elementAt(loop);
    ...
    public static Instruction findInstruction(long id, Vector<Instruction> instrs) throws Exception {
        Instruction ret = null;
        for (int loop = 0; loop < instrs.size(); loop++) {
            ret = (Instruction) instrs.elementAt(loop);
            if (ret.getInstructionId() == id) {
     ...
     * @param offSetclass MethodInfo
     * @param instrs
     * @return
     */
    public static int findInstructionIndex(int offSet, Vector<Instruction> instrs) throws Exception {
        Instruction ret = null;
        for (int loop = 0; loop < instrs.size(); loop++) {
            ret = instrs.elementAt(loop);
            if (ret.getOffSet() == offSet) {
