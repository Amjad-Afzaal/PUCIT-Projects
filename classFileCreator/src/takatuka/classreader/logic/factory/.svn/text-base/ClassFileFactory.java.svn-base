...
public class ClassFileFactory {
    private static final ClassFileFactory classFileFactory = new
            ClassFileFactory();

    protected ClassFileFactory() {
        super();
    }

    static ClassFileFactory getInstanceOf() {
        return classFileFactory;
    }

    protected ClassFile createClassFile(MultiplePoolsFacade cstPool, 
            String uniqueName) throws Exception {
        return new ClassFile(cstPool, uniqueName);
    }

    protected MethodInfo createMethodInfo(ClassFile myClass) {
        return new MethodInfo(myClass);
    }

    protected FieldInfo createFieldInfo(ClassFile myClass) {
        return new FieldInfo(myClass);
    }

//---------- todo move them to different later own.
