...
public class MiscInstrs {
    ...
    protected void returnExecute(MultiplePoolsFacade pOne, 
...
//todo            throw new VerifyErrorExt(Messages.STACK_INVALID);
     ...
     */
    protected void instanceofInstruction(MultiplePoolsFacade pOne, 
            VerificationInstruction inst) {
        VerificationFrameFactory frameFactory = FrameFactoryPlaceHolder.getInstanceOf().getFactory();
        Type ref = stack.pop();
        if (!ref.isReference()) {
        ...
        if (index == 0 || index >= pOne.getCurrentSize(CPTagValues.CONSTANT_Class)) {
