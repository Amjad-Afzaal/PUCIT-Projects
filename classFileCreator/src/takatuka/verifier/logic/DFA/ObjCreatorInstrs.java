...
import takatuka.classreader.dataObjs.constantPool.DoubleInfo;
import takatuka.classreader.dataObjs.constantPool.FloatInfo;
import takatuka.classreader.dataObjs.constantPool.IntegerInfo;
import takatuka.classreader.dataObjs.constantPool.LongInfo;
...
import takatuka.classreader.dataObjs.constantPool.base.InfoBase;
import takatuka.classreader.dataObjs.constantPool.base.RegularInfoBase;
...
public class ObjCreatorInstrs {
    ...
    protected void ldcInstruction(VerificationInstruction inst,
        ...
        try {
            ...
            if ((!isCategoryOne && type.isReference())
                    || (!isCategoryOne
                    && type.getType() != Type.DOUBLE
                    && type.getType() != Type.LONG)) {
    ...
    protected Type getLDCTagType(VerificationInstruction instr) {
        ...
        MultiplePoolsFacade cp = instr.getMethod().getClassFile().getConstantPool();
        int cpIndex = instr.getOperandsData().intValueSigned();
        InfoBase refInfo = (InfoBase) cp.get(cpIndex);
        
        if (refInfo instanceof StringInfo) {
            tag = Type.STRING;
            isReference = true;
        } else if (refInfo instanceof FloatInfo) {
            tag = Type.FLOAT;
        } else if (refInfo instanceof IntegerInfo) {
            tag = Type.INTEGER;
        } else if (refInfo instanceof DoubleInfo) {
            tag = Type.DOUBLE;
        } else if (refInfo instanceof LongInfo) {
            tag = Type.LONG;
        } else {
    ...
    protected void newArrayInstruction(VerificationInstruction inst) {
        ...
        Type aType = frameFactory.createType(arrayType);
        aType.setIsArray();
        stack.push(aType);
    }
    ...
    protected int convertATypeToTagValue(int aType) {
        if (aType == FieldTypes.TYPE_JBOOLEAN || aType == FieldTypes.TYPE_JCHAR || aType == FieldTypes.TYPE_JBYTE
            ...
            return CPTagValues.CONSTANT_Integer;
        } else if (aType == FieldTypes.TYPE_JFLOAT) {
            return CPTagValues.CONSTANT_Float;
        } else if (aType == FieldTypes.TYPE_JDOUBLE) {
            return CPTagValues.CONSTANT_Double;
        } else if (aType == FieldTypes.TYPE_JLONG) {
            return CPTagValues.CONSTANT_Long;
        } else {
