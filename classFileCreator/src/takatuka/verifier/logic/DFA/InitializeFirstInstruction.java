...
import takatuka.classreader.logic.constants.CPTagValues;
...
public class InitializeFirstInstruction {
    ...
    public static Frame createFrame(MethodInfo method, Vector localVariables) {
        ...
        if (localVariables == null) {
            localVar = createLocalVariablesOfFirstInstruction(codeAtt.getMaxLocals().intValueUnsigned(),
                    methodDescriptor,
                    isStatic, null, method.getClassFile().getConstantPool());
        } else {
     ...
     * @return 
     */
    public static int getType(String descStr, int fromIndex,
            Type returnType, MultiplePoolsFacade cp) {
//        returnType.isReference = false;
//        returnType.isArray = false;
        char ctype = descStr.charAt(fromIndex);
        int retIndex = fromIndex;
        if (ctype == ';') {
            return getType(descStr, fromIndex + 1, returnType, cp);
            ...
            retIndex = returnType.setReferenceType(descStr, fromIndex, cp);
        } else if (ctype == '[') {
            while (descStr.charAt(retIndex) != '[') {
                retIndex++;
            }
            int toRet = getType(descStr, retIndex + 1, returnType, cp);
    ...
    public static Vector<Type> getMethodParametersTypes(String methodDesc, boolean isStatic,
        ...
        while (cur < strParam.length()) {
            ...
            cur = getType(strParam, cur, temp, cp);
            ret.addElement(temp);
    ...
    public static int getMethodParamertersSize(String methodDesc, boolean isStatic, MultiplePoolsFacade cp) {
        Vector<Type> typesVec = getMethodParametersTypes(methodDesc, isStatic, cp);
    ...
    public static LocalVariables createLocalVariablesOfFirstInstruction(int maxLocalSize,
            String descStr, boolean isStatic, Type thisTypeForNonStaticMethods, MultiplePoolsFacade cp) {
        VerificationFrameFactory frameFactory = FrameFactoryPlaceHolder.getInstanceOf().getFactory();
        ...
        for (int loop = 0; loop < maxLocalSize /*|| descStr.charAt(cur) != ')'*/;) {
            ...
            if (descStr.charAt(cur) != ')') {
                cur = getType(descStr, cur, toAdd, cp);
            }
