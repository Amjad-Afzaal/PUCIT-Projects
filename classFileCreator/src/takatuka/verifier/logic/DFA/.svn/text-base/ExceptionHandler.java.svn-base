...
public class ExceptionHandler {
    ...
    public static void exceptionsThrownByAnInstruction(VerificationInstruction instr,
        ...
        try {
            ...
            MultiplePoolsFacade pOne = currentMethod.getClassFile().getConstantPool();

            while (it.hasNext()) {
                ClassFile cFile = it.next();
                Oracle oracle = Oracle.getInstanceOf();
                int classInfoIndex = oracle.getClassInfoByName(cFile.getFullyQualifiedClassName(),
                        pOne);
                if (classInfoIndex != -1) {
                    thisClasses.add(classInfoIndex);
                }
            }
            Vector<Long> jumpTo = new Vector<Long>();
            athrowJumpPC(pOne, thisClasses, instr.getOffSet(), currentMethod, exceptionRecord);
        } catch (Exception d) {
            d.printStackTrace();
     ...
     *
     */
    protected final void athrowInstruction(MultiplePoolsFacade pOne,
            VerificationInstruction currentInstr) {
        Oracle oracle = Oracle.getInstanceOf();
        String methodName = oracle.getMethodOrFieldString(currentInstr.getMethod());
        ...
        /**
         * stack should only have a single reference
         */
        stack.clear();
        stack.push(ref);
        try {
            ...
            athrowJumpPC(pOne, inputCatchTypes, currentPC, currentMethod, excepTionInfo);
     ...
     * @throws Exception
     */
    private static void athrowJumpPC(MultiplePoolsFacade pOne, Vector<Integer> inputCatchTypes, int currentPC,
        ...
        Oracle oracle = Oracle.getInstanceOf();
        pOne = currentMethod.getClassFile().getConstantPool();
        ...
        for (int catchLoop = 0; catchLoop < inputCatchTypes.size(); catchLoop++) {
            for (int loop = 0; loop < exceptTableEntry.size(); loop++) {
                ...
                System.out.println(" --sf saf- safas --- " + expTbl);
                ...
                /**            System.out.println("asdfasf asfas "+instr);
