 ...
 * @version 1.0
 */
public class Type extends FieldTypes {
    ...
    public int setReferenceType(String name, int refIndex, MultiplePoolsFacade cp) {
        int cur = refIndex;
        int start = cur + 1;
        String refName = name.substring(start, name.indexOf(";", start));
        int localType = Oracle.getInstanceOf().getClassInfoByName(refName, cp);
